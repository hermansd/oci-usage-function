...
wallet_path = os.environ['TNS_ADMIN']
work_report_dir = "/tmp/work_report_dir"
...
##########################################################################
    ...
    #parser.add_argument('-ip', action='store_true', default=True, dest='instance_principals', help='Use Instance Principals for Authentication')
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if val == 0:
                ...
                cursor.execute(sql)
                logging.info("   Table OCI_USAGE created")
            else:
            ...
            if val == 0:
                logging.info("   Column TAGS_DATA does not exist in the table OCI_USAGE, adding...")
            ...
            if val == 0:
                logging.info("   Column TENANT_ID does not exist in the table OCI_USAGE, adding...")
            ...
            if val == 0:
                logging.info("   Column TAG_SPECIAL does not exist in the table OCI_USAGE, adding...")
            ...
            if val == 0:
                logging.info("   Column TAG_SPECIAL2 does not exist in the table OCI_USAGE, adding...")
            ...
            if val == 0:
                logging.info("   Table OCI_USAGE_TAG_KEYS was not exist, creating")
                ...
                cursor.execute(sql)
                logging.info("   Table OCI_USAGE_TAG_KEYS created")
            else:
                logging.info("   Table OCI_USAGE_TAG_KEYS exist")
            ...
            if val == 0:
                logging.info("   Table OCI_USAGE_STATS was not exist, creating")
                ...
                cursor.execute(sql)
                logging.info("   Table OCI_USAGE_STATS created")
        ...
        logging.error("Error manipulating database at check_database_table_structure_usage() - " + str(e) + "\n")
        raise SystemExit

    except Exception as e:
        raise Exception("Error manipulating database at check_database_table_structure_usage() - " + str(e))
...
def check_database_table_structure_resource(connection):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # if table not exist, create it
            if val == 0:
                logging.info("   Table OCI_RESOURCES was not exist, creating")
                ...
                cursor.execute(sql)
                logging.info("   Table OCI_RESOURCES created")
            else:
                logging.info("   Table OCI_RESOURCES exist")
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if val == 0:
                ...
                logging.info("   Index OCI_USAGE_1IX does not exist for table OCI_USAGE, adding...")
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if val == 0:
                ...
                logging.info("   Index OCI_COST_1IX does not exist for table OCI_COST, adding...")
...
def update_cost_stats(connection, tenant_name):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            connection.commit()
            logging.info("   Merge Completed, " + str(cursor.rowcount) + " rows merged" + get_time_elapsed(start_time))
...
def update_cost_reference(connection, tag_special_key, tag_special_key2, tenant_name):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            logging.info("   Merging statistics from OCI_COST...")
            ...
            if tag_special_key:
                ...
                logging.info("   Handling Tag Special Key '" + tag_special_key + "'")
            ...
            if tag_special_key2:
                ...
                start_time = time.time()
                # run merge to OCI_COST_REFERENCE for the tag special key
                logging.info("   Handling Tag Special Key '" + tag_special_key2 + "'")
...
def update_usage_reference(connection, tag_special_key, tag_special_key2, tenant_name):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            logging.info("\nMerging statistics into OCI_COST_REFERENCE ...")
            logging.info("   Merging statistics from OCI_USAGE...")
        ...
        logging.error("Error manipulating database at update_usage_reference() - " + str(e) + "\n")
        raise SystemExit

    except Exception as e:
        raise Exception("Error manipulating database at update_usage_reference() - " + str(e))
...
def update_public_rates(connection, tenant_name):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            logging.info("   Update Completed, " + str(num_rows) + " rows updated." + get_time_elapsed(start_time))

    except cx_Oracle.DatabaseError as e:
        logging.error("Error manipulating database at update_public_rates() - " + str(e) + "\n")
        raise SystemExit

    except requests.exceptions.ConnectionError as e:
        logging.error("Error connecting to billing metering API at update_public_rates() - " + str(e))
        logging.error("Please check you can connect to " + api_url + "partNumber=B90000")

    except Exception as e:
        raise Exception("Error manipulating database at update_public_rates() - " + str(e))
...
def update_usage_stats(connection, tenant_name):
    try:
        ...
        logging.error("Error manipulating database at update_usage_stats() - " + str(e) + "\n")
        raise SystemExit

    except Exception as e:
        raise Exception("Error manipulating database at update_usage_stats() - " + str(e))
...
def update_tenant_id_if_null(connection, tenant_name, short_tenant_id):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if cnt > 0:
                logging.info("   Update TENANT_ID on OCI_USAGE... " + str(cnt) + " rows to update... updating max 1,000,000 per execution")
                ...
                connection.commit()
                logging.info("   Update Completed, " + str(cursor.rowcount) + " rows updated" + get_time_elapsed(start_time))
            ...
            if cnt > 0:
                logging.info("   Update TENANT_ID on OCI_COST..." + str(cnt) + " rows to update... updating max 1,000,000 per execution")
        ...
        logging.error("Error manipulating database at update_tenant_id_if_null() - " + str(e) + "\n")
        raise SystemExit

    except Exception as e:
        raise Exception("Error manipulating database at update_tenant_id_if_null() - " + str(e))
...
def check_database_table_structure_cost(connection, tag_special_key, tag_special_key2, tenant_name):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # if table not exist, create it
            if val == 0:
                logging.info("   Table OCI_COST was not exist, creating")
                ...
                cursor.execute(sql)
                logging.info("   Table OCI_COST created")
            else:
            ...
            # if columns TENANT_ID not exist, create it
            if val == 0:
                logging.info("   Column TENANT_ID does not exist in the table OCI_COST, adding...")
            ...
            if val == 0:
                logging.info("   Column TAG_SPECIAL does not exist in the table OCI_COST, adding...")
            ...
            if val == 0:
                logging.info("   Column TAG_SPECIAL2 does not exist in the table OCI_COST, adding...")
            ...
            if val == 0:
                logging.info("   Table OCI_COST_TAG_KEYS was not exist, creating")
                ...
                cursor.execute(sql)
                logging.info("   Table OCI_COST_TAG_KEYS created")
            else:
            ...
            if val == 0:
                ...
                cursor.execute(sql)
                logging.info("   Table OCI_COST_STATS created")
            ...
            if val == 0:
                logging.info("   Column COST_MY_COST_OVERAGE does not exist in the table OCI_COST_STATS, adding...")
            ...
            if val == 0:
                logging.info("   Table OCI_COST_REFERENCE was not exist, creating")
                ...
                cursor.execute(sql)
                logging.info("   Table OCI_COST_REFERENCE created")
            ...
            if val == 0:
                logging.info("   Table OCI_INTERNAL_COST was not exist, creating")
                ...
                cursor.execute(sql)
                logging.info("   Table OCI_INTERNAL_COST created")
            else:
                logging.info("   Table OCI_INTERNAL_COST exist")

    except cx_Oracle.DatabaseError as e:
        logging.error("Error manipulating database at check_database_table_structure_cost() - " + str(e) + "\n")
        raise SystemExit

    except Exception as e:
        raise Exception("Error manipulating database at check_database_table_structure_cost() - " + str(e))
...
def check_database_table_structure_price_list(connection, tenant_name):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # if table not exist, create it
            if val == 0:
                logging.info("   Table OCI_PRICE_LIST was not exist, creating")
                ...
                cursor.execute(sql)
                logging.info("   Table OCI_PRICE_LIST created")
            ...
            if val == 0:
                logging.info("   Column TENANT_ID does not exist in the table OCI_PRICE_LIST, adding...")
                sql = "alter table OCI_PRICE_LIST add (TENANT_ID VARCHAR2(100))"
                cursor.execute(sql)

                logging.info("   truncating OCI_PRICE_LIST to make PK changes")
                sql = "truncate table OCI_PRICE_LIST"
                cursor.execute(sql)
                logging.info("   Modifying OCI_PRICE_LIST primary key to include TENANT_ID..")
                ...
                cursor.execute(sql)
                logging.info("   Table OCI_PRICE_LIST Changes Completed.")

    except cx_Oracle.DatabaseError as e:
        logging.error("Error manipulating database at check_database_table_price_list() - " + str(e) + "\n")
        raise SystemExit

    except Exception as e:
        raise Exception("Error manipulating database at check_database_table_price_list() - " + str(e))
...
def check_database_table_structure_load_status(connection):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # if table not exist, create it
            if val == 0:
                logging.info("   Table OCI_LOAD_STATUS was not exist, creating")
                ...
                cursor.execute(sql)
                logging.info("   Table OCI_LOAD_STATUS created")
            else:
                logging.info("   Table OCI_LOAD_STATUS exist")

    except cx_Oracle.DatabaseError as e:
        logging.error("Error manipulating database at check_database_table_structure_load_status() - " + str(e) + "\n")
        raise SystemExit

    except Exception as e:
        raise Exception("Error manipulating database at check_database_table_structure_load_status() - " + str(e))


#########################################################################
# Load Cost File
##########################################################################
def load_cost_file(connection, object_storage, object_file, max_file_id, tenancy, compartments, file_num, total_files):
    start_time = time.time()
    ...
    try:
        ...
        if str(max_file_id) != "None":
            if file_id <= str(max_file_id):
                logging.info("   Skipping   file " + file_name_full + " - " + str(file_size_mb) + " MB, " + file_time + ", #" + str(file_num) + "/" + str(total_files) + ", File already loaded")
                return num_files

        # if file id enabled, check
        if os.environ['fileid']:
            if file_id != os.environ['fileid']:
                logging.info("   Skipping   file " + file_name_full + " - " + str(file_size_mb) + " MB, " + file_time + ", #" + str(file_num) + "/" + str(total_files) + ", File Id " + os.environ['fileid'] + " filter specified")
                return num_files

        # check file date
        if os.environ['filedate']:
            if file_time <= os.environ['filedate']:
                logging.info("   Skipping   file " + file_name_full + " - " + str(file_size_mb) + " MB, " + file_time + ", #" + str(file_num) + "/" + str(total_files) + ", Less then specified date " + os.environ['filedate'])
                return num_files

        path_filename = work_report_dir + '/' + filename
        logging.info("   Processing file " + file_name_full + " - " + str(file_size_mb) + " MB, " + file_time + ", #" + str(file_num) + "/" + str(total_files))
        ...
        with gzip.open(path_filename, 'rt') as file_in:
            ...
            with connection.cursor() as cursor:
                ...
                for row in csv_reader:
                    ...
                    for (key, value) in row.items():
                        if 'tags' in key and len(value) > 0:
                            ...
                            # if tagspecial
                            if os.environ['tagspecial']:
                                if keyadj == os.environ['tagspecial']:
                            ...
                            # if tagspecial2
                            if os.environ['tagspecial2']:
                                if keyadj == os.environ['tagspecial2']:
                                    if len(valueadj) < 4000:
                ...
                connection.commit()
                logging.info("   Completed  file " + file_name_full + " - " + str(num_rows) + " Rows Inserted" + get_time_elapsed(start_time))
        ...
        if data:
            with connection.cursor() as cursor:
                ...
                cursor.executemany(sql, data)
                connection.commit()
                logging.info(", " + str(len(data)) + " Tags Merged.")
        else:
            logging.info("")
        ...
        logging.error("load_cost_file() - Error manipulating database - " + str(e) + "\n")
        raise SystemExit

    except Exception as e:
        logging.error("load_cost_file() - Error Download Usage and insert to database - " + str(e))
        raise SystemExit
...
# Load Usage File
##########################################################################
def load_usage_file(connection, object_storage, object_file, max_file_id, tenancy, compartments, file_num, total_files):
    start_time = time.time()
    start_time_str = get_current_date_time()
    num_files = 0
    num_rows = 0
    try:
        ...
        if str(max_file_id) != "None":
            if file_id <= str(max_file_id):
                logging.info("   Skipping   file " + o.name + " - " + str(round(o.size / 1024 / 1024)) + " MB, " + file_time + ", #" + str(file_num) + "/" + str(total_files) + ", File already loaded")
                return num_files

        # if file id enabled, check
        if os.environ['fileid']:
            if file_id != os.environ['fileid']:
                logging.info("   Skipping   file " + o.name + " - " + str(round(o.size / 1024 / 1024)) + " MB, " + file_time + ", #" + str(file_num) + "/" + str(total_files) + ", File Id " + os.environ['fileid'] + " filter specified")
                return num_files

        # check file date
        if os.environ['filedate']:
            if file_time <= os.environ['filedate']:
                logging.info("   Skipping   file " + o.name + " - " + str(round(o.size / 1024 / 1024)) + " MB, " + file_time + ", #" + str(file_num) + "/" + str(total_files) + ", Less then specified date " + os.environ['filedate'])
                return num_files

        path_filename = work_report_dir + '/' + filename
        logging.info("   Processing file " + o.name + " - " + str(round(o.size / 1024 / 1024)) + " MB, " + file_time + ", #" + str(file_num) + "/" + str(total_files))
        ...
        with gzip.open(path_filename, 'rt') as file_in:
            ...
            with connection.cursor() as cursor:
                ...
                # commit
                connection.commit()
                logging.info("   Completed  file " + o.name + " - " + str(num_rows) + " Rows Inserted" + get_time_elapsed(start_time))
        ...
        logging.error("load_usage_file() - Error manipulating database - " + str(e) + "\n")
        raise SystemExit

    except Exception as e:
        logging.error("load_usage_file() - Error Download Usage and insert to database - " + str(e))
        raise SystemExit
#########################################################################
...
def main_process(signer):
    ...
    # Start
    ############################################
    logging.info("Starts at " + get_current_date_time())
    ...
    try:
        ...
        if os.environ['proxy']:
            identity.base_client.session.proxies = {'https': os.environ['proxy']}
        ...
        logging.error("Error extracting compartments section - " + str(e) + "\n")
        raise SystemExit
    ...
    secret_client = oci.secrets.SecretsClient(config={'region': 'us-phoenix-1'}, signer=signer)

    # Retrieve secret
    dbpasswd = read_secret_value(secret_client, os.environ['secret_ocid'])
  
    # autonomous database dependencies..
    autonomous_database_id = os.environ['db_ocid']
    generate_autonomous_database_wallet_details = oci.database.models.GenerateAutonomousDatabaseWalletDetails(password=dbpasswd)
    autonomous_db = oci.database.DatabaseClient({'region': 'us-phoenix-1'}, signer=signer)
    
    # create local directories..
    if not os.path.exists(wallet_path):
       os.mkdir(wallet_path)
       logging.debug('Creating Wallet Directory: ' +  wallet_path)


    # download db client credential package..
    with open(wallet_path + '/wallet.zip', 'wb') as f:
        logging.info('trying to load walletfile')
     ...
     # update sqlnet.ora..
        ...
        sqlnet_filedata = sqlnet_filedata.replace('?/network/admin', wallet_path)
        with open(wallet_path + '/sqlnet.ora', 'w') as sqlnet_file:
            sqlnet_file.write(sqlnet_filedata)
        with open(wallet_path + '/sqlnet.ora', 'r') as sqlnet_file:
            sqlnet_filedata = sqlnet_file.read()
            logging.debug(sqlnet_filedata)

    logging.info('Wallet created moving on to connecting to Databse ' + os.environ['db_dsn'])

    try:
        with cx_Oracle.connect(user=os.environ['db_user'], password=dbpasswd, dsn=os.environ['db_dsn'], encoding="UTF-8") as connection:

            # Open Cursor
            logging.info("Working in database " + os.environ['db_dsn'])
            with connection.cursor() as cursor:
                ...
                check_database_table_structure_cost(connection, os.environ['tagspecial'], os.environ['tagspecial2'], tenancy.name)
                ...
                # for usage and cost
                ###############################
                logging.info("Checking Last Loaded Files... started at " + get_current_date_time())
                ...
                logging.info("Completed Checking at " + get_current_date_time())

            ############################################
            # Download Usage, cost and insert to database
            ############################################

            logging.info("Connecting to Object Storage Service...")

            object_storage = oci.object_storage.ObjectStorageClient(config, signer=signer)
            if os.environ['proxy']:
                object_storage.base_client.session.proxies = {'https': os.environ['proxy']}
            logging.info("   Connected")
            ...
            #############################
            usage_num = 0
            if os.environ['skip_usage'] != False:
                logging.info("Handling Usage Report... started at " + get_current_date_time())
                ...
                logging.info("Total " + str(total_files) + " usage files found to scan...")
                for index, object_file in enumerate(objects.objects, start=1):
                    usage_num += load_usage_file(connection, object_storage, object_file, max_usage_file_id, tenancy, compartments, index, total_files)
                logging.info("   Total " + str(usage_num) + " Usage Files Loaded, conmpleted at " + get_current_date_time())

            #############################
            # Update oci_usage_stats if
            # there were files
            #############################
            if usage_num > 0 or os.environ['force']:
                update_usage_stats(connection, tenancy.name)
                update_usage_reference(connection, os.environ['tagspecial'], os.environ['tagspecial2'], tenancy.name)

            #############################
            # Handle Cost Usage
            #############################
            cost_num = 0
            if os.environ['skip_cost'] != False:
                logging.info("Handling Cost Report... started at " + get_current_date_time())
                ...
                total_files = len(objects.objects)
                logging.info("Total " + str(total_files) + " cost files found to scan...")
                for index, object_file in enumerate(objects.objects, start=1):
                    cost_num += load_cost_file(connection, object_storage, object_file, max_cost_file_id, tenancy, compartments, index, total_files)
                logging.info("   Total " + str(cost_num) + " Cost Files Loaded, completed at " + get_current_date_time())
            ...
            # there were files
            #############################
            if cost_num > 0 or os.environ['force']:
                update_cost_stats(connection, tenancy.name)
                update_price_list(connection, tenancy.name)
                update_cost_reference(connection, os.environ['tagspecial'], os.environ['tagspecial2'], tenancy.name)
                if os.environ['skip_rate'] != False:
                    update_public_rates(connection, tenancy.name)
                update_tenant_id_if_null(connection, tenancy.name, short_tenant_id)

    except cx_Oracle.DatabaseError as e:
        logging.error("Error manipulating database - " + str(e) + "\n")

    except Exception as e:
        logging.error("Error appeared - " + str(e))

    ############################################
    # print completed
    ############################################

    return "Completed at " + get_current_date_time()